...
class homeListAPIView(APIView):
    def post(self, request):
        ...
        if form.is_valid():
            ...
            formsido = form.data.get('siDo')

            acc_ty_cd = defaultdict(int)
            aslt_vtr_cd = defaultdict(int)
            info=dict(totalAccient=0, acc_year=form.data.get('searchYear'), sido=formsido, casualties=0, dth_dnv_cnt=0, se_injured_cnt=0, sl_injured_cnt=0, wnd_dnv_cnt=0,
            ...
            for gugun in GUGUN_CHOICES[int(formsido)]: #구/군에 따른 반목 ex) 서울시 : 강남구, 성북구 ....
                params ={'ServiceKey' : formservicekey, 'searchYear' : form.data.get('searchYear'), 'siDo' : formsido, 'guGun' : gugun, 'type' : 'json', 'numOfRows' : '9999', 'pageNo' : '1' }
                ...
                if r.status_code == requests.codes.ok: #코드 상태 확인
                    ...
                    if form.data.get('select') == '1':  #요약페이지 일 시 info 칼럼 삭제
                    ...
                    for cnt in range(gutotalcnt):
                        ...
                        if form.data.get('select') == '2':   #상세 페이지
